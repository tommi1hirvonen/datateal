...
public interface IWorkspaceManagementRepository
{
    ...
    Task<WorkspaceEntity> CreateAsync(string name, string slug, string? description, CancellationToken ct = default);
    Task<WorkspaceEntity?> UpdateAsync(Guid id, string name, string slug, string? description, CancellationToken ct = default);
